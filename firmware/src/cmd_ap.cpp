...
const char* const HOW_LINES[] = {
    ...
    "  These four are the only characteristics the bike exposes.",
    "  Everything — settings, PIN, fingerprint — rides this one pipe.",
    " ",
...
const char* const SETUP_LINES[] = {
    ...
    " every time something tries to bond. You read it passively. Only the",
    " module's TX -> your RX carries data; the link is read-only and cannot",
    " control anything on the bike. Find TX and GND on the module (RX is not",
    " needed); see docs/ble_solder_points.png for the exact pads.",
    " ",
    " A-1. ON THE DASHBOARD (no computer needed):",
    "   The screen IS your serial terminal — the Diagnostics page shows",
    "   whatever the module prints.",
    "   1. Solder the module's TX -> the display's UART-header RX, GND->GND.",
    "      (Powering the screen from the bike is a SEPARATE pair of labeled",
    "      5V/GND pads on the mainboard, not this debug port.)",
    "   2. On the dashboard: Settings > Diagnostics.",
    "   3. Trigger a bond from anything (this dashboard, LightBlue on iOS,",
    "      nRF Connect on Android). Enter any 6-digit number; it'll fail.",
    "   4. The real 6-digit passkey scrolls onto the Diagnostics page.",
    "   5. Settings > Command Prompt > Start AP, browse to 192.168.4.1/,",
    "      type:  setblepin XXXXXX  then tap 'Re-pair bike'. Done — no",
    "      computer ever involved.",
    " ",
    " A-2. WITH A USB-UART ADAPTER (handy before the screen is built):",
    "   1. Module TX -> adapter RX, GND -> GND.",
    "      Cheap CP2102 / FTDI / CH340 adapter, any of them work.",
    "   2. Open a serial terminal at 115200 baud, 8N1, no flow control.",
    ...
    "   3. Power-cycle the bike; you'll see boot text incl. 'Reevo>READY'.",
    "   4. Trigger a bond attempt from anything; enter any 6-digit number.",
    "   5. The terminal prints the real passkey. Type it into the web prompt:",
    "        setblepin XXXXXX   then tap 'Re-pair bike' on Settings>Bluetooth.",
    " ",
